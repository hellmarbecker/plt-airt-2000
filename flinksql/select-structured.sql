...
select
    ...
    if(session_id = '', cast(null as int), cast(session_id as int)) as session_id,
    if(aircraft_id = '', cast(null as int), cast(aircraft_id as int)) as aircraft_id,
    hex_ident,
    if(flight_id = '', cast(null as int), cast(flight_id as int)) as flight_id,
    date_message_generated,
    time_message_generated,
    date_message_logged,
    time_message_logged,
    callsign,
    if(altitude = '', cast(null as int), cast(altitude as int)) as altitude,
    if(ground_speed = '', cast(null as double), cast(ground_speed as double)) as ground_speed,
    if(track = '', cast(null as double), cast(track as double)) as track,
    ...
    if(vertical_rate = '', cast(null as int), cast(vertical_rate as int)) as vertical_rate,
    squawk,
    if(alert = '', cast(null as int), cast(alert as int)) as alert,
    if(emergency = '', cast(null as int), cast(emergency as int)) as emergency,
    if(spi = '', cast(null as int), cast(spi as int)) as spi,
    if(is_on_ground = '', cast(null as int), cast(is_on_ground as int)) as is_on_ground 
from cte2
