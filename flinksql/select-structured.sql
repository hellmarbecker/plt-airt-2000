...
select
    ...
    `timestamp`,
    message_type,
    try_cast(transmission_type as int) as transmission_type,
    try_cast(session_id as int) as session_id,
    try_cast(aircraft_id as int) as aircraft_id,
    hex_ident,
    try_cast(flight_id as int) as flight_id,
    date_message_generated,
    time_message_generated,
    date_message_logged,
    time_message_logged,
    callsign,
    try_cast(altitude as int) as altitude,
    try_cast(ground_speed as double) as ground_speed,
    try_cast(track as double) as track,
    try_cast(latitude as double) as latitude,
    try_cast(longitude as double) as longitude,
    try_cast(vertical_rate as int) as vertical_rate,
    squawk,
    try_cast(alert as int) as alert,
    try_cast(emergency as int) as emergency,
    try_cast(spi as int) as spi,
    try_cast(is_on_ground as int) as is_on_ground 
from cte2
